...
(* Exercises *)
Lemma map_rev_lem: forall (X Y: Type) (x: X) (f : X -> Y) (l : list X),
...
Module Exercises.
  ...
  Example eq_map: fold_map (fun x => x) [1;2;3] = map (fun x => x) [1;2;3].
  Proof. reflexivity. Qed.
  ...
  Fixpoint nth_error {X : Type}
           (l : list X) (n : nat) : option X
    := match l with
       | [] => None
